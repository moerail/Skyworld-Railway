(() => {
  ...
  ['{driver} requested {mode} MA. Result: {state} ({reason}); executable: {executable}.',
    '{driver} 申请 {mode} MA。结果：{state}（{reason}）；可执行：{executable}。',
    '{driver} a demandé une MA {mode}. Résultat : {state} ({reason}) ; exécutable : {executable}.',
    '{driver} が {mode} MA を要求。結果：{state}（{reason}）；実行可能：{executable}。'],
  ['{driver} released forward MA reservation. Train occupancy retained.',
    '{driver} 释放车前 MA 预约。车身占用保留。',
    '{driver} a libéré la réservation MA en avant. Occupation du train conservée.',
    '{driver} が前方 MA 予約を解放。車両在線は保持。'],
  ['{operator} approved SR for {train} to {node}.',
    '{operator} 批准 {train} 的 SR 至 {node}。',
    '{operator} a autorisé le SR de {train} jusqu’à {node}.',
    '{operator} が {train} の SR を {node} まで承認。'],
  ['Executable MA unavailable for {train} ({reason}). Stop at the last confirmed EoA.',
    '{train} 的可执行 MA 已不可用（{reason}）。须在最后确认的 EoA 前停车。',
    'MA exécutable indisponible pour {train} ({reason}). Arrêt à la dernière EoA confirmée.',
    '{train} の実行可能 MA が失効（{reason}）。最終確認済み EoA までに停止。'],
  ['Approve SR to this point','批准 SR 至此处','Autoriser le SR jusqu’ici','この地点まで SR を許可'],
  ['SR approval','SR 审批','Autorisation SR','SR 承認'],
  ['Approve SR','批准 SR','Autoriser le SR','SR を承認'],
  ['SR approved','SR 已批准','SR autorisé','SR 承認済み'],
  ...
  codes.push(...[["VALIDATING","Validating route","正在核验进路","Vérification du parcours","進路確認中"],["NO_PENDING_SR","No pending SR request","没有待审批的 SR 申请","Aucune demande SR en attente","承認待ちの SR 要求なし"],["INVALID_TARGET","Invalid SR target","SR 目标无效","Cible SR invalide","SR 目標が無効"],["STOP_FIRST","Stop the train first","请先停车","Arrêtez d'abord le train","先に停止してください"],["SR_TARGET_TOO_FAR","SR target too distant","SR 目标超出距离上限","Cible SR trop éloignée","SR 目標が遠すぎます"],["TARGET_UNREACHABLE","Target unreachable in rail graph","轨道图中目标不可达","Cible inaccessible dans le graphe","軌道図上で目標に到達できません"],["ROUTE_MISMATCH","Safe path differs from SR target route","安全进路与 SR 目标不一致","Le parcours sûr diffère de la cible SR","安全な進路が SR 目標と一致しません"],["ROUTE_UNAVAILABLE","No safe SR route currently available","当前无可分配的安全 SR 进路","Aucun parcours SR sûr disponible actuellement","現在、安全な SR 進路を割り当てられません"],["REQUEST_ID_CONFLICT","Request ID conflict","请求编号冲突","Conflit d'identifiant de requête","要求 ID 競合"]]);
