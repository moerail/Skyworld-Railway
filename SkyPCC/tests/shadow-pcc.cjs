...
let pointState = 'straight', stale = false, posts = 0;
...
function snapshot() {
  const now = Date.now();
  return { schemaVersion: 1, serviceStatus: 'AVAILABLE', graphRevision: 1, serverTimeMillis: now,
    ...
    shadowMa: { version: 4, simulationOnly: true, executable: false, session: 'test', sequence: 1, emittedAtMillis: now - (stale ? 10000 : 0),
      ...
      authorities: [{ trainId, state: 'ALLOCATED_SHADOW', reason: 'RESOURCE_CONFLICT', signedRemainingMeters: 140,
        eoaEdgeId: 'e1', eoaOffsetMeters: 75, path: [] }] } };
}
const server = http.createServer(async (req, res) => {
  const json = value => { res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify(value)); };
  if (req.url === '/api/v1/config') return json({ updateMode: 'poll', pollIntervalMillis: 250, controlEnabled: true });
  if (req.url === '/api/v1/graph') { nodes[2].state = pointState; return json({ revision: 1, nodes, edges }); }
  if (req.url === '/api/v1/trains') return json(snapshot());
  if (req.url === '/api/v4/switch') {
  ...
  if (!['index.html', 'styles.css', 'app.js', 'day_logo.png', 'night_logo.png'].includes(asset)) { res.statusCode = 404; return res.end(); }
